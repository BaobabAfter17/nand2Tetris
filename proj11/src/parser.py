...
def compile_subroutine_body(subroutine_type):
    ...
    if subroutine_type == "method":     # Align THIS to its argument 0
        code_writer.write_push(ARGUMENT, 0)
        code_writer.write_pop(POINTER, 0)

    elif subroutine_type == "constructor":  # Alloc and return this
...
def compile_string():
    string = get_token_and_advance()[1:-1]  # remove double quotes
    length = len(string)
...
def compile_subroutine_call_in_different_class():
    ...
    if type_ is not None: # name is varName
        ...
        code_writer.write_push(segment, index)  # push var
        n_args += 1
...
def compile_subroutine_call_in_same_class():
    subroutine_name = get_token_and_advance()
    advance()
    code_writer.write_push(POINTER, 0)  # push this
    n_args = compile_expression_list()
    advance()
...
def lookup_var(name):
    ...
    @Return:
    type_: string, type of variable or None if not found.
...
def parse_single_file(input_filename, elem_list_from_tokenizer):
    """Parse a single .jack file to .vm file."""
